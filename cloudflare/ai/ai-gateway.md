...
Don’t need caching, rate limiting, or the request log? The plain [Workers AI binding](workers-ai.md) (`Cloudflare.Workers.AI`) calls the same models directly, with no gateway resource — and its `ai.model({...})` produces the same `LanguageModel` Layer, so you can start there and add the gateway later without touching handler code.
...
`Cloudflare.AI.QueryGateway(Gateway)` returns a typed client whose methods are wrapped in Effect — `run` for raw inference, `getUrl` for the gateway endpoint, `getLog` / `patchLog` for the request log, and `model({...})` for building a `LanguageModel` layer.
...
Every prop on `Cloudflare.AI.Gateway` maps to an update API call — no replacement, no downtime. A production-grade config might look like:
...
- A `Cloudflare.AI.Gateway` resource with caching, logging, and (when you want them) rate limiting and auth.
...
The natural next step is to give the model **memory** — wrapping the same `LanguageModel` with `Chat.Service` and storing the conversation in a Durable Object’s `state.storage`, so every session is resumable across requests, restarts, and hibernation.

For the wider API surface — `generateObject`, `Toolkit`, structured outputs, and how the same `LanguageModel` composes inside an HTTP API or RPC handler — see the [Effect AI guide](effect-ai.md).
