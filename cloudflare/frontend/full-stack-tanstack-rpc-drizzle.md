...
- **[TanStack Start](frontends.md)** — the React frontend, deployed as a
  Cloudflare Worker + assets via `Cloudflare.Website.Vite`.
- **[Effect RPC](../apis/effect-rpc.md)** — a typed backend served by a separate
  `Cloudflare.Workers.RpcWorker`.
- **[Drizzle + Neon Postgres](../data/shared-database.md)** — reached through a
...
[Effect RPC guide](../apis/effect-rpc.md) for a deeper tour of `Rpc.make` and
...
its own walk-through in [Shared database across stages](../data/shared-database.md).
:::
...
export default class Backend extends Cloudflare.Workers.RpcWorker<Backend>()(
  "Backend",
  { main: import.meta.filename, workersDev: false, schema: TodoRpcs },
  Effect.gen(function* () {
...
resource into an SSR Worker (see [Frontend frameworks](frontends.md)):
...
export default Alchemy.Stack(
  ...
  Effect.gen(function* () {
    ...
    // public URL of its own (`workersDev: false`), so there's nothing to return.
...
- [Effect RPC](../apis/effect-rpc.md) — the RPC server/client model in depth,
  including streaming and Durable Object backends.
- [Frontend frameworks](frontends.md) — `Cloudflare.Website.Vite` and
  `Cloudflare.Website.StaticSite` for every framework.
- [Shared database across stages](../data/shared-database.md) — the `pr-*` branch
