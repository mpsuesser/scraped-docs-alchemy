...
SolidStart’s SSR server bundle uses Node APIs at runtime — the `nodejs_compat` compatibility flag covers that, and it is enabled by default for every Worker, so there is nothing to pass.
...
Both env channels of the Vite resource apply unchanged: `VITE_` -prefixed entries are inlined into the client bundle as `import.meta.env.<KEY>` at build time, and everything else attaches to the SSR Worker as runtime bindings:
