---
url: https://alchemy.run/neon/guides/production-auth
title: "Production Auth"
description: "Configure trusted origins, SMTP, OAuth callbacks, and verification before launching Managed Better Auth."
...
latter using [Custom domains](custom-domains.md).
...
| Auth was disabled outside Alchemy | Preserve `neon_auth` and state; use [State and recovery](state-recovery.md) rather than dropping user data to re-enable it |

## Related guides

- [Setup](../setup.md) — credential and account prerequisites.
- [Auth reference](https://alchemy.run/providers/neon/reference/auth#auth) — supported settings and lifecycle.
- [Upload tutorial](../tutorial.md) — development application and its production limits.
