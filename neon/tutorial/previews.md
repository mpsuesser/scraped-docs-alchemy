---
url: https://alchemy.run/neon/tutorial/previews
title: "Fork a preview and clean up"
description: "Keep preview writes isolated from the parent, verify inheritance, and destroy only owned resources."
...
Run this with the same stage and retained local state used for deployment. `forceDestroy: true` intentionally erases the tutorial bucket's files before removing its branch and project. If state is missing or ownership recovery fails, follow [State and recovery](../guides/state-recovery.md) before making further changes.
