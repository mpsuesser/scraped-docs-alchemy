...
Your `--profile admin` AWS credentials need IAM admin rights for this stack: it creates an `OpenIDConnectProvider` and an IAM `Role`. Run `alchemy profile edit --profile admin --add AWS` and choose a credential (SSO/OIDC, access keys, etc.) that’s authorized for IAM administration.
