...
Everything on one account, with large pushes hashed by dynamically loaded Workers so nothing leaves Cloudflare:
...
- A Worker with `nodejs_compat`, a 300 s CPU limit, a `worker_loader` binding for the hasher, and the shared secret bound as a secret.
- Two Durable Object namespaces on that Worker, one per repository and one for the registry, each with SQLite storage.

Nothing runs until a request arrives, and a repository that receives no traffic costs its storage.
...
A registry on D1 replicates name lookups instead of serving them from one region:
...
A custom domain is a Worker option, so clone URLs read `https://git.example.com/acme/web.git`. [Custom domains & routes](../../cloudflare/networking/custom-domains.md) covers it, and the [application example](https://github.com/alchemy-run/alchemy/tree/main/examples/cloudflare-git-service) serves a browser UI and Git on the same origin.
