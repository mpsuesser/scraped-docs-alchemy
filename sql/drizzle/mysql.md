---
url: https://alchemy.run/sql/drizzle/mysql
title: "MySQL"
...
`Drizzle.MySQL` provides typed, Effect-native queries for compatible MySQL databases, independently of where your application runs. Choose a database in [SQL databases](../databases.md) and supply its connection through your runtime’s binding or configuration.
...
The [Cloudflare walkthrough](../../cloudflare/data/drizzle.md#use-planetscale-instead) supplies Hyperdrive’s connection string; [PlanetScale](../../planetscale/guides/drizzle.md) owns database and password configuration. On Workers, the client selects text-protocol queries and eval-free row parsers; see [Workers defaults](../effect-sql/mysql.md#workers-defaults).
...
Choose a MySQL-compatible database and deployment guide in [SQL databases](../databases.md). For tagged-template queries without an ORM, use [Effect SQL: MySQL](../effect-sql/mysql.md).
