---
url: https://alchemy.run/sql/effect-sql/d1
title: "D1"
...
The client (and its prepared-statement cache) is built lazily on the first query of an execution and torn down when the event settles — see [Connection lifecycle](lifecycle.md).
...
The layer provides two tags from one per-execution client: the generic `SqlClient.SqlClient`, and `@effect/sql-d1` ’s `D1Client` for code that needs it — including drizzle’s `effect-d1` driver. Swapping `SQL.D1Layer` for [`SQL.PostgresLayer`](postgres.md#provide-as-a-service) or [`SQL.MySQLLayer`](https://alchemy.run/sql/effect-sql/mysql#provide-as-a-service) moves the same service graph to another database unchanged.
...
- [Connection lifecycle](lifecycle.md) — when the client is built and torn down.
- [Drizzle on D1](../drizzle/d1.md) — the same driver behind a typed schema.
- [D1](../../cloudflare/data/d1.md) — the database resource: replication, jurisdictions, imports, cloning.
