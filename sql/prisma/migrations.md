---
url: https://alchemy.run/sql/prisma/migrations
title: "Migrations"
description: "How Prisma.Contract and Prisma.Migrate run Prisma's contract emission, migration planning, and graph-ordered apply inside alchemy deploy."
...
Both [PSL-first and TypeScript-first contracts](contracts.md) use this
...
line, so unlike [Drizzle](../drizzle/migrations.md) there are no other
dialects to choose between — for those databases, use Drizzle or the
raw [Effect SQL clients](../effect-sql/postgres.md). Alchemy pins the
exact RC version in its catalog; expect the RC line to keep moving
until 8.0.0 final.

## Where next

- [Postgres](postgres.md) — the runtime query API.
- [Cloudflare integration](../../cloudflare/data/prisma.md) — the full contract, database, and Worker setup.
- [SQL databases](../databases.md) — database choices and canonical deployment guides.
- [Effect SQL: Migrations](../effect-sql/migrations.md) — the plain
  `.sql` migration contract used by Drizzle and the raw clients.
- [Prisma Postgres](../../prisma.md) — provision the database with the Prisma
  platform provider.
