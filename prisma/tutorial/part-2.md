...
`Prisma.Compute` bundles the handler into a Bun archive and deploys it without a Dockerfile. `main` points to the file that default-exports this class. The Project already owns the `main` branch, so select it by `branchGitName`.
...
The API depends on the same Project definition. Yielding it from both files still creates only one Project in this Stack.
...
Prisma checks this route when starting the service. It verifies that the HTTP process is responding, independently of the database added in Part 3.
...
Replace the host with `apiUrl`. The response should be `{"ok":true}`. Request `/missing` to check the 404 response, or use `curl -X POST` to check the 405 response.

## Next

[Part 3: Query Postgres](part-3.md) binds a database connection to the API and runs a real SQL query.
